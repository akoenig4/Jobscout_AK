...
class Executer:
            ...
            try:
                response = self.executions_table.query(
                    ...
                    KeyConditionExpression=boto3.dynamodb.conditions.Key(
                        'next_exec_time').eq(current_time),
    ...
    def update_next_execution(self, task_id, current_time, segment):
        ...
        new_next_exec_time = self.calculate_next_exec_time(
    ...
    def add_to_history_data(self, task_id, exec_time, status, retries):
        ...
        print(f"Added task_id={task_id} to historydata table with status '{
              status}' and {retries} retries.")
...
class Master:
    def __init__(self, schedule_instances: int = 1):
        ...
        # This ensures ceil(schedule_instances / 4)
        self.executer_count = (schedule_instances + 3) // 4
...
if __name__ == "__main__":
    ...
    master_thread = threading.Thread(
        target=run_master_in_background, args=(master,))
    # Set daemon to True so it exits when the main thread exits
